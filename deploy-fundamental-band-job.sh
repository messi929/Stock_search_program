...
#
# ⚠️ KRX 펀더멘탈 엔드포인트는 야간점검 시 빈 응답 → 낮 시간 스케줄(토 15:00 KST).
#    잡은 초반 연속 빈 응답이면 조기 중단(낭비 방지).
#
...
JOB_ARGS="-m,jobs.backfill_fundamental_band"
...
# ── Cloud Run Job ─────────────────────────────
# pykrx 300종목 × 1s = ~5분 + fetch → 타임아웃 30분 여유.
