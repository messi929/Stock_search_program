...
# 훅 (첫 줄 = 노출의 전부 — 좋아요·팔로우가 여기서 갈린다)
- 첫 줄은 **셋업·인사·종목 소개로 시작하지 마라.** 가장 센 수치나 가장 날카로운 긴장 하나를
  **첫 줄에 곧바로** 던진다. 배경 설명·자기소개는 뒤로.
- 좋은 훅은 '궁금해서 멈추게' 만든다. 세 가지 중 하나로 연다:
  · 모순 — "거래량이 3배 터졌는데 외국인은 5일째 순매도야"
  · 의외의 한 수치 — "52주 신고가인데 PER은 업종 평균 밑이야"
  · 통념 뒤집기 — "다들 싸다는데, 이 PER이 오히려 함정일 수 있어"
- 첫 줄은 **짧게 한 방으로**(대략 한 문장). 숫자는 자료에 있는 것만, 뭉개지 말고 그대로 박는다.
- 바로 다음 줄에서 '왜 그런지 뒤에서 까줄게' 신호를 줘서 계속 읽게 만든다(설명·근거는 뒷줄/뒷파트).
- ❌ 약한 훅(그 첫 줄은 폐기하고 다시 쓴다): "요즘 ○○ 관심 많지", "오늘은 ○○ 얘기해볼게",
  "○○라는 종목이 있어", "심상치 않다/움직임이 있다" 같은 두루뭉술한 도입."""
...
def over_limit_parts(parts: list[str]) -> list[str]:
    """THREADS_MAX(500자)를 넘는 파트의 '순번(길이)' 목록. 비면 전부 발행 가능.

    Threads는 글 1개(=파트 1개)가 500자를 넘으면 발행 자체가 막힌다. 넘는 초안을
    검수 큐에 올려봐야 복사→발행 단계에서 튕기므로, 생성 단계에서 폐기한다
    (JEON 결정 2026-07-18). 각 생성기는 finalize_thread 직후 이걸 호출해
    비어있지 않으면 None을 반환한다(= 그 초안은 생성되지 않음)."""
    return [f"{i}번째({len(p)}자)" for i, p in enumerate(parts, 1) if len(p) > THREADS_MAX]
