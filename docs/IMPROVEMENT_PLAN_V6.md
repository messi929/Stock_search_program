...
## 구현 상태 (2026-04-10 업데이트)

| Phase | 항목 | 상태 |
|-------|------|------|
| 1-1 | 다기간 수익률 (backtest 전면 개편) | ✅ |
| 1-2 | score_history Firestore 저장 | ✅ |
| 1-3 | 벤치마크 대비 알파 | ✅ |
| 1-4 | UI 시그널 성적표 | ✅ (v6.1 다기간 탭 + α배지 + 점수별 성과) |
| 2-1 | supply_history + 연속 매수 일수 | ✅ |
| 2-2 | 수급 강도 등급 | ✅ |
| 2-3 | 시장 수급 게이지 API + UI | ✅ (v6.1 프로그레스바) |
| 2-4 | pykrx 기관 유형별 분리 | ⬜ (pykrx 정상화 대기) |
| 3-1 | yfinance 추가 필드 수집 | ✅ |
| 3-2 | schemas 필드 추가 | ✅ |
| 3-3 | buy_score 가치 팩터 강화 | ✅ |
| 3-4 | quality 카테고리 추가 | ✅ |
| 3-5 | UI 모달 펀더멘탈/수급 탭 | ✅ (v6.1 4탭: 기본\|펀더멘탈\|수급\|리스크) |
| 4-1 | 포지션 사이징 | ✅ |
| 4-2 | 상관관계 + 섹터 편중 | ✅ |
| 4-3 | 시장 레짐 감지 | ✅ |
| 4-4 | 레짐별 전략 가중치 | ✅ |
| 4-5 | 섹터 편중 경고 | ✅ |
| 4-UI | 포트폴리오 리스크 대시보드 | ✅ (v6.1 상관계수+섹터분포+편중경고) |
| 4-UI | 시장 레짐 인디케이터 | ✅ (v6.1 헤더 배지) |
| 4-UI | 포지션 사이징 표시 | ✅ (v6.1 리스크 탭) |
| 5-1 | 종목 비교 API + UI | ✅ (v6.1 카드형 비교뷰) |
| 5-2 | 스마트 알림 | ✅ |
| 5-3 | 섹터 로테이션 맵 API + UI | ✅ (v6.1 자금흐름 그리드) |
| 5-4 | 실적 캘린더 | ⬜ (외부 API 의존) |

> ⬜ = 외부 의존성 대기 (pykrx 정상화, DART API 등)
>
> v6.0 백엔드 완료 (2026-04-07) → v6.1 프론트엔드 완료 (2026-04-10)
