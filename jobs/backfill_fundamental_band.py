...
def run_backfill(
    limit: int = 300, years: int = 5, sleep_sec: float = 1.0, dry_run: bool = False
) -> dict:
    ...
    for i, t in enumerate(tickers):
        ...
        if df is None:
            empty_streak += 1
            # 초반 연속 빈 응답 = KRX 펀더멘탈 점검 추정 → 조기 중단(낮에 재실행).
            if i < 10 and empty_streak >= 6:
                logger.error(
                    "[band] 초반 연속 빈 응답 — KRX 펀더멘탈 점검 추정, 중단(낮에 재실행)"
                )
                return {"computed": len(bands), "saved": 0, "aborted": True}
        ...
        if band:
            bands[t] = band
        if (i + 1) % 50 == 0:
            logger.info(f"[band] 진행 {i + 1}/{len(tickers)} (수집 {len(bands)})")
        time.sleep(sleep_sec)
    ...
    if dry_run:
        ...
        saved = save_fundamental_band(bands)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    res = run_backfill(limit=a.limit, years=a.years, sleep_sec=a.sleep, dry_run=a.dry_run)
